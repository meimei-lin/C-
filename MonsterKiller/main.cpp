...
void Character::actionAttack(Character *other) {
...
protected:
    void actionAttack(Character* other) override {
        ...
        cout<<"monster get damage:"<<attack<<endl;
    }
...
class Angrybird : public Monster{
protected:
    void actionAttack(Character* other) override {
        cout<<"Angrybird choose attack"<<endl;
        int damage=(int)(attack*0.4);
        other->damaged(damage);
        cout<<"player get damage:"<<damage<<endl;
    }
    void actionDefence(Character* other) override{
        cout<<"Angrybird choose defence"<<endl;
        int defend=(int)(defence*0.4);
        this->defended(defend);
        cout<<"Angrybird get defence:"<<defence<<endl;
    }
    void actionSkill(Character* other) override{
        cout<<"Angrybird choose skill"<<endl;
        int damage=(int)(magicAttack*0.3);
        other->damaged(damage);
        mp-=skillCost;
        cout<<"Angrybird cost  "<<skillCost<<"mp to use skill"<<endl;
        cout<<"Damage to other:"<<damage<<endl;
    }
public:
    Angrybird(int hp, int mp, int atk, int dfc, int mAtk, int skCost) : Monster(hp, mp, atk, dfc, mAtk, skCost) {
        name = "Angrybird";
    }

    void reward(Player* player) override {}
};
class Fatpig:public Monster{
protected:
    void actionAttack(Character* other) override {
        cout << "Fatpig choose attack" << endl;
        int damage = (int) (attack * 0.5);
        other->damaged(damage);
        cout << "Fatpig attack damage:" << damage << endl;
    }
    void actionDefence(Character* other) override {
        cout<<"Fatpig choose defence"<<endl;
        int defend=(int)(defence*0.4);
        this->defended(defend);
        cout<<"Fatpig get defence:"<<defend<<endl;
    }
    void actionSkill(Character* other) override {
        cout << "Fatpig choose skill" << endl;
        int damage = (int) ( magicAttack * 0.3);
        other->damaged(damage);
        mp -= skillCost;
        cout << "Fatpig cost  " << skillCost << "mp to use skill" << endl;
        cout << "Damage to other" << damage << endl;
    }
public:
    Fatpig(int hp, int mp, int atk, int dfc, int mAtk, int skCost) : Monster(hp, mp, atk, dfc, mAtk, skCost) {
        name = "Fatpig";
    }
...
public:
    ...
    Game() {
        // Create player
        player = new Player(100, 100, 50, 30, 35, 25);

        // Create monster
        monsters.push_back(new Angrybird(50,100,30,15,35,55));
        monsters.push_back(new Fatpig(100,150,50,50,40,50));
    }
